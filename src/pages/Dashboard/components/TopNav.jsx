      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 ml-4">
          <div className="text-right hidden md:block">
            <p className="text-xs font-bold text-on-surface-variant uppercase tracking-tighter leading-none">{JSON.parse(localStorage.getItem('user'))?.nombre || 'Usuario'}</p>
            <p className="text-[10px] text-stone-400 dark:text-on-surface-variant font-label uppercase tracking-widest">{JSON.parse(localStorage.getItem('user'))?.rol || 'Invitado'}</p>
          </div>
